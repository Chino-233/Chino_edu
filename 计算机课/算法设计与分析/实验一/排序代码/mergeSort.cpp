...
//归并排序
void merge(vector<int> &arr, int left, int mid, int right, const chrono::steady_clock::time_point &startTime, const double timeoutSeconds)
{
...
void mergeSortImpl(vector<int> &arr, int left, int right, const chrono::steady_clock::time_point &startTime, const double timeoutSeconds)
{
    auto currentTime = chrono::steady_clock::now();
    double elapsedSeconds = chrono::duration<double>(currentTime - startTime).count();
    if (elapsedSeconds > timeoutSeconds)
    ...
    if (left < right)
    {
        ...
        mergeSortImpl(arr, left, mid, startTime, timeoutSeconds);
        mergeSortImpl(arr, mid + 1, right, startTime, timeoutSeconds);

        merge(arr, left, mid, right, startTime, timeoutSeconds);
    }
}

bool mergeSort(vector<int> &arr, const chrono::steady_clock::time_point &startTime, const double timeoutSeconds)
{
    mergeSortImpl(arr, 0, arr.size() - 1, startTime, timeoutSeconds);
...
double testSortingFile(const string &filePath)
{
    ...
    bool completed = mergeSort(data, startTime, timeoutSeconds);
