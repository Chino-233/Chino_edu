...
public:
    Increase(int x) : value(x) {} //构造函数
     //友元函数形式重载前增量，类内声明, ++lhs, lhs自增，返回lhs引用
    friend Increase &operator++(Increase &lhs);  
    //友元函数形式重载后增量, 类内声明，lhs++ ，返回lhs的值，再加1
...
//前加加实现，++lhs, C语言中int a = 10, 支持++(++a), 即返回的是空间，对象引用
...
//后加加实现，lhs++, C语言中int a = 10, 不支持(a++)++，即返回的是a的值，a再加1
Increase operator++(Increase &lhs, int)
{
    Increase temp(lhs);   //返回原来的值，先做原对象备份
    lhs.value++;                //返回前加1
